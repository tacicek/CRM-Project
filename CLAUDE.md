...
- **Portal/marketplace bileşenleri** (CookieBanner, TrackingProvider, partner
  ...
  kasıtlıdır.
- **TÜRKÇE METİN YASAK — ne UI'da ne kodda.**
  - **UI (kullanıcı/müşteri görünür): her zaman ALMANCA.** JSX içi yazılar, başlık/alt-başlık,
    buton/label/placeholder, `toast`/hata mesajı, boş-durum, PDF (`@react-pdf`), e-posta
    şablonları, public sayfalar (OfferView, termin). Karışık/Türkçe görürsen ("Pozisyon ekle,
    … seç, gönder" gibi) Almanca'ya çevir. Müşteriye giden ekranda Türkçe = kabul edilemez hata.
  - **Kod + yorumlar: İngilizce ya da Almanca — Türkçe DEĞİL.** Yeni kod yorumu, değişken/fonksiyon
    adı, log/dev notu İngilizce veya Almanca yaz. Türkçe yorum ekleme. (Commit mesajı serbest.)
    Repoda eski Türkçe yorumlar var — dokunduğun dosyada karşına çıkarsa İngilizce/Almanca'ya çevir;
    sırf çeviri için ayrı tarama yapma (fix/feature ile birlikte).
