/**
 * Locale-aware replacement for _shared/emailLayout.ts.
 *
 * Same exports, same visual shell — the only difference is that
 * `wrapEmailDocument(inner, locale)` emits `<html lang="{locale}">` instead of a
 * hardcoded `lang="de"`. The locale parameter is optional and defaults to 'de',
 * so existing call sites keep working unchanged during the migration.
 *
 ...
 *
 * @param innerHtml Body markup to place inside the shell.
 * @param locale    Customer locale; drives the `lang` attribute. Defaults to 'de'.
 */
