...
const logStep = (step: string, details?: unknown) => {
  ...
  console.log(`[n8n-lead-webhook] ${step}${detailsStr}`);
};
 ...
 * Falls back to length mismatch early exit (safe because length is not secret-sensitive here).
 */
...
function verifyN8nSecret(req: Request): boolean {
  ...
  // If no secret configured, block all requests in production
  if (!webhookSecret) {
    logStep("Warning: N8N_WEBHOOK_SECRET not configured, rejecting request");
    return false;
  }
  
  // Check x-webhook-secret header (constant-time comparison)
  const headerSecret = req.headers.get("x-webhook-secret");
  if (headerSecret && timingSafeEqual(headerSecret, webhookSecret)) return true;
  
  // Check Authorization bearer token (constant-time comparison)
...
serve(async (req) => {
  ...
  try {
    ...
    const leadData = {
      ...
      // Status — admin must verify before match-lead / distribution
      ...
      Promise.resolve().then(async () => {
        try {
          ...
          await resend.emails.send({
            ...
            html: wrapEmailDocument(n8nCustomerInner),
          });
