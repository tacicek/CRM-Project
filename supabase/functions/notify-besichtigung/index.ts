...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // ── Locales ────────────────────────────────────────────────────────────────
    // Two recipients, two languages. The payload carries offerId, so the customer's language is
    // read from offers.language rather than added to the request contract. The firma copy stays
    // on the company's own dashboard language.
    ...
    const customerLocale = toLocale(offerRow?.language);
            ...
            <div style="background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin: 20px 0;">
              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  ...
                  <td style="padding: 8px 0; font-weight: 600; text-align: right;">${escapeHtml(request.offerTitle)}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; color: #64748b;">${tCompany("email.besichtigungRequest.offerAmount")}:</td>
                  <td style="padding: 8px 0; font-weight: 600; text-align: right;">${formatCurrency(request.offerTotal, companyLocale)}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; color: #64748b;">${tCompany("common.customer")}:</td>
                  <td style="padding: 8px 0; text-align: right;">${escapeHtml(request.customerName)}</td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td style="padding: 8px 0; text-align: right;">
                    <a href="tel:${encodeURIComponent(request.customerPhone)}" style="color: #3b82f6;">${escapeHtml(request.customerPhone)}</a>
                  </td>
                </tr>
                ` : ""}
              </table>
    ...
    // Create in-app notification for the company — shown in the firm's dashboard, so company locale
    if (request.companyId) {
      const { error: notificationError } = await supabase
        ...
        .insert({
          ...
          body: `${request.customerName} wünscht eine Besichtigung am ${formatDateLocale(request.besichtigungDate, companyLocale)}${request.besichtigungTime ? ` um ${request.besichtigungTime} Uhr` : ""} für "${request.offerTitle}".`,
          metadata: {
    ...
    // Send confirmation email to customer — CUSTOMER language (offers.language)
          ...
          <div style="background: #f8fafc; padding: 30px; border: 1px solid #e2e8f0; border-top: none;">
            ...
            <div style="background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 20px; margin: 20px 0;">
              <table style="width: 100%; border-collapse: collapse;">
                ...
                <tr>
                  ...
                  <td style="padding: 8px 0; font-weight: 600; text-align: right;">${escapeHtml(request.companyName)}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; color: #64748b;">${tCustomer("email.besichtigungRequest.offerAmount")}:</td>
                  <td style="padding: 8px 0; font-weight: 600; text-align: right;">${formatCurrency(request.offerTotal, customerLocale)}</td>
                </tr>
              </table>
            </div>
            ...
            <div style="background: #dbeafe; border: 1px solid #3b82f6; border-radius: 8px; padding: 16px; margin: 20px 0;">
              <p style="margin: 0; color: #1e40af;">
                ...
                ${tCustomer("email.besichtigungRequest.nextStepsBody", { companyName: `<strong>${escapeHtml(request.companyName)}</strong>` })}
              </p>
            </div>

            <p style="margin-bottom: 0; color: #64748b; font-size: 14px;">
              ${tCustomer("common.regards")}<br>
              <strong>${escapeHtml(isCompanyEmail ? request.companyName : tCustomer("common.teamSignature", { appName: getAppName() }))}</strong>
            </p>
          </div>
