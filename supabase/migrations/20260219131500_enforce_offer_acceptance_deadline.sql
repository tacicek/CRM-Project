-- Enforce acceptance deadline:
-- An offer can be accepted at latest one day before service_date.
-- If valid_until exists, the effective deadline is the earlier of:
--   1) valid_until
--   2) service_date - 1 day
...
BEGIN
  -- Read current offer dates for business rule checks.
  SELECT service_date, valid_until
  INTO v_service_date, v_valid_until
  FROM public.offers
  WHERE access_token = offer_access_token;

  -- If accepting: enforce deadline (latest one day before service_date).
