-- =============================================================================
-- FIX: update_offer_by_token — tam Auftrag INSERT restore
--
-- 20260416000002 ve 20260606120000 migration'larındaki bozuk INSERT'i düzeltir.
-- Sorunlar:
--   1. status = 'confirmed' → auftrag_status enum'unda yok (doğrusu: 'geplant')
--   2. NOT NULL alanlar eksikti: title, customer_name, scheduled_date
--
-- Bu migration şunları birleştirir:
--   • 20260416: status whitelist, NOT FOUND check, agb_ip_address ignored
--   • 20260606: terminal status guard (accepted/rejected bloklar)
--   • 20260411: tam Auftrag INSERT + takvim randevusu + lead status güncelleme
-- =============================================================================
...
CREATE OR REPLACE FUNCTION public.update_offer_by_token(
  ...
  new_agb_ip_address              text DEFAULT NULL  -- intentionally ignored; Edge Function sets this server-side
)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- ── 2. Offer bilgilerini oku ─────────────────────────────────────────────
  SELECT o.status, o.service_date, o.valid_until, o.id, o.company_id, o.lead_id
  ...
  -- Kabul edilmiş veya reddedilmiş teklif üzerinde status değişikliği yapılamaz
  IF new_status IS NOT NULL AND v_status = ANY(TERMINAL_STATUSES) THEN
    RETURN false;
  END IF;

  -- ── 4. Kabul deadline kontrolü ───────────────────────────────────────────
  IF new_status = 'accepted' THEN
    v_acceptance_deadline := v_valid_until;
    IF v_service_date IS NOT NULL THEN
      IF v_acceptance_deadline IS NULL OR
         (v_service_date - INTERVAL '1 day')::date < v_acceptance_deadline THEN
  ...
  SET
    ...
    -- agb_ip_address: kasıtlı olarak yok — Edge Function header'dan alır, client'tan gelmez
  ...
  IF new_status = 'accepted' AND v_offer_id IS NOT NULL THEN
    ...
    CROSS JOIN LATERAL (
      SELECT COALESCE(o.service_end_time::time,
                      (start_calc.start_time + interval '2 hour')::time) AS end_time
    ) end_calc
    CROSS JOIN LATERAL (
      SELECT
    ...
    -- ── 6b. Auftrag otomatik oluştur (idempotent — tüm zorunlu alanlar) ──
    ...
    SELECT
      ...
      o.id,
      o.lead_id,
      '',   -- trigger generate_auftrag_nummer bunu otomatik doldurur
      COALESCE(NULLIF(TRIM(o.title), ''), 'Auftrag'),
      TRIM(CONCAT(
      ...
      NULLIF(TRIM(CONCAT(
        ...
        COALESCE(l.from_house_number, ''),
        CASE
          WHEN l.from_plz IS NOT NULL
          THEN ', ' || l.from_plz || ' ' || COALESCE(l.from_city, '')
          ELSE ''
        END
      )), ''),
      NULLIF(TRIM(CONCAT(
        ...
        CASE
          WHEN l.to_plz IS NOT NULL
          THEN ', ' || l.to_plz || ' ' || COALESCE(l.to_city, '')
          ELSE ''
        END
      )), ''),
    ...
    -- ── 6c. lead_distributions.status = 'job_confirmed' ─────────────────
...
COMMENT ON FUNCTION public.update_offer_by_token IS
  'Müşteri teklifini access token ile kabul/red/görüntüleme durumu günceller. '
  'Güvenlik: new_status whitelist (viewed/accepted/rejected), terminal guard (accepted/rejected bloklar), '
  'agb_ip_address client''tan gelene kadar görmezden gelir. '
  'Kabul edilince idempotent olarak: takvim randevusu + Auftrag (tüm alanlarla) oluşturur, '
