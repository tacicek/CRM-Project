-- =============================================================================
-- #7 — update_offer_by_token: appointment + auftrag'ı DOĞUMDA linkle
--
-- 20260607120000 sürümünün üzerine: kabul akışında oluşturulan (veya zaten var
-- olan) service appointment'ın id'si yakalanır ve auftrag.appointment_id'ye yazılır.
-- Böylece tek kaynak bağı baştan kurulur.
-- =============================================================================
...
BEGIN
  ...
  IF new_status = 'accepted' AND v_offer_id IS NOT NULL THEN
    ...
    -- ── 6a-2. İlgili service appointment id'sini yakala (yeni veya mevcut) ─
    SELECT a.id INTO v_appointment_id
    FROM public.appointments a
    WHERE a.offer_id = v_offer_id AND a.appointment_type = 'service'
    ORDER BY a.created_at ASC
    LIMIT 1;

    -- ── 6b. Auftrag otomatik oluştur (idempotent + appointment link) ──────
    INSERT INTO public.auftraege (
      company_id, offer_id, lead_id, appointment_id,
      auftrag_nummer, title, customer_name, customer_email, customer_phone,
      from_address, to_address, scheduled_date, scheduled_time, description, status
    )
    SELECT
      o.company_id, o.id, o.lead_id, v_appointment_id,
      '',
      COALESCE(NULLIF(TRIM(o.title), ''), 'Auftrag'),
      TRIM(CONCAT(COALESCE(o.customer_first_name, ''), ' ', COALESCE(o.customer_last_name, ''))),
      o.customer_email, o.customer_phone,
      NULLIF(TRIM(CONCAT(
        COALESCE(l.from_street, ''), ' ', COALESCE(l.from_house_number, ''),
        CASE WHEN l.from_plz IS NOT NULL THEN ', ' || l.from_plz || ' ' || COALESCE(l.from_city, '') ELSE '' END
      )), ''),
      NULLIF(TRIM(CONCAT(
        COALESCE(l.to_street, ''), ' ', COALESCE(l.to_house_number, ''),
    ...
    WHERE o.access_token = offer_access_token
      AND NOT EXISTS (
        SELECT 1 FROM public.auftraege a WHERE a.offer_id = o.id AND a.deleted_at IS NULL
      );

    -- ── 6c. lead_distributions.status = 'job_confirmed' ─────────────────
    UPDATE public.lead_distributions
    SET status = 'job_confirmed', responded_at = COALESCE(responded_at, NOW()), updated_at = NOW()
    WHERE lead_id = v_lead_id AND company_id = v_company_id AND status = 'accepted';
...
COMMENT ON FUNCTION public.update_offer_by_token IS
  'Müşteri teklifini token ile günceller. Kabul edilince idempotent olarak service '
  'appointment + Auftrag oluşturur ve ikisini appointment_id ile linkler (tek kaynak). '
