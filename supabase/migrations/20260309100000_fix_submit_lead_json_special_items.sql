...
-- Fix: jsonb_array_elements_text fails when special_items is JSON null (not SQL NULL).
-- The old check `lead_data->'special_items' IS NULL` only catches SQL NULL (missing key),
-- but when JS sends `null` the JSONB stores it as JSON null ('null'::jsonb), which IS NOT
-- SQL NULL. So the ELSE branch runs jsonb_array_elements_text on a scalar → 22023 error.
-- Fix: also check jsonb_typeof to ensure the value is actually an array before extracting.
...
BEGIN
  INSERT INTO public.leads (
    ...
    -- FIX: use jsonb_typeof to safely check for array before extracting elements.
    -- JSON null ('null'::jsonb) is NOT SQL NULL, so IS NULL check alone is insufficient.
    CASE
