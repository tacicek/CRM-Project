-- Add verification columns to leads table
ALTER TABLE public.leads 
ADD COLUMN IF NOT EXISTS verified_by UUID REFERENCES auth.users(id),
ADD COLUMN IF NOT EXISTS verified_at TIMESTAMP WITH TIME ZONE,
ADD COLUMN IF NOT EXISTS rejection_reason TEXT,
ADD COLUMN IF NOT EXISTS admin_notes TEXT,
ADD COLUMN IF NOT EXISTS spam_score INTEGER DEFAULT 0,
ADD COLUMN IF NOT EXISTS ip_address TEXT;

-- Update default status to pending_verification
ALTER TABLE public.leads ALTER COLUMN status SET DEFAULT 'pending_verification';

-- Create index for faster filtering
CREATE INDEX IF NOT EXISTS idx_leads_status ON public.leads(status);
CREATE INDEX IF NOT EXISTS idx_leads_created_at ON public.leads(created_at DESC);

-- Comment on status values
COMMENT ON COLUMN public.leads.status IS 'pending_verification, verified, rejected, distributed, completed';

-- Drop the old trigger that auto-distributes (we'll create a new one that only triggers on verified status)
DROP TRIGGER IF EXISTS on_new_lead_match ON public.leads;

-- Create new trigger that only fires when status changes to verified
CREATE OR REPLACE FUNCTION public.trigger_match_verified_lead()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
DECLARE
  supabase_url text;
  service_role_key text;
  request_id bigint;
BEGIN
  -- Only trigger when status changes to 'verified'
  IF NEW.status = 'verified' AND (OLD.status IS NULL OR OLD.status != 'verified') THEN
    supabase_url := current_setting('app.settings.supabase_url', true);
    service_role_key := current_setting('app.settings.service_role_key', true);
    
    IF supabase_url IS NULL OR supabase_url = '' THEN
      supabase_url := 'https://yrehijkkurxywwhrxriu.supabase.co';
    END IF;

    SELECT net.http_post(
      url := supabase_url || '/functions/v1/match-lead',
      headers := jsonb_build_object(
        'Content-Type', 'application/json',
        'Authorization', 'Bearer ' || COALESCE(service_role_key, current_setting('request.jwt.claim.sub', true))
      ),
      body := jsonb_build_object('lead_id', NEW.id)
    ) INTO request_id;

    RAISE LOG '[trigger_match_verified_lead] Triggered match-lead for verified lead %, request_id: %', NEW.id, request_id;
  END IF;

  RETURN NEW;
END;
$function$;

-- Create trigger for verified leads
CREATE TRIGGER on_lead_verified
  AFTER UPDATE ON public.leads
  FOR EACH ROW
  EXECUTE FUNCTION public.trigger_match_verified_lead();