-- Migration: Calculate token_cost at lead creation time in submit_lead_json
-- Reason: Leads were defaulting to 10 tokens until match-lead ran.
-- Now token price is estimated immediately using service_catalog + pricing_settings.
...
BEGIN
  ...
  -- 1. Fetch base_token_cost from service_catalog
  ...
  -- 3. Determine effective rooms (from rooms or m² fallback)
  ...
  -- 4. Size multiplier lookup
  IF v_effective_rooms IS NOT NULL AND v_effective_rooms > 0 THEN
    FOR v_key, v_val IN SELECT * FROM jsonb_each_text(v_size_multipliers) LOOP
      IF v_key LIKE '%+' THEN
        IF v_effective_rooms >= replace(v_key, '+', '')::NUMERIC THEN
          v_size_mult := v_val::NUMERIC;
        END IF;
      ELSIF v_key LIKE '%-%' THEN
        IF v_effective_rooms >= split_part(v_key, '-', 1)::NUMERIC
           AND v_effective_rooms <= split_part(v_key, '-', 2)::NUMERIC THEN
          v_size_mult := v_val::NUMERIC;
        END IF;
      ELSE
        IF ABS(v_effective_rooms - v_key::NUMERIC) < 0.5 THEN
          v_size_mult := v_val::NUMERIC;
        END IF;
      END IF;
    END LOOP;
  ...
  INSERT INTO public.leads (
    ...
    CASE WHEN lead_data->>'preferred_date' IS NOT NULL AND lead_data->>'preferred_date' != ''
         THEN (lead_data->>'preferred_date')::DATE ELSE NULL END,
    ...
    (lead_data->>'piano_weight_kg')::NUMERIC,
