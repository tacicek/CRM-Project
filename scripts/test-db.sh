...
set -e
if [ "$RC" -ne 0 ] || [ "$RC2" -ne 0 ] || [ "$RC3" -ne 0 ] || [ "$RC4" -ne 0 ] || [ "$RC5" -ne 0 ] || [ "$RC6" -ne 0 ]  || [ "$RC7" -ne 0 ] || [ "$RC8" -ne 0 ]; then
  echo "DB integration suite FAILED (assertions rc=$RC, auftrag-contract rc=$RC2, customer-360 rc=$RC3, rollen rc=$RC4, offerte-version rc=$RC5, nachtrag rc=$RC6, pipeline rc=$RC7, finanzen rc=$RC8)"; exit 1
fi
