...
const addDaysIso = (iso: string, days: number): string => {
  // Compute via UTC: local parse + UTC format was shifting the calendar day
  // (in CH UTC+2, "datum + 30" resulted in 29 days).
...
export default function RechnungDetail() {
  ...
  // Position helpers — betrag is calculated automatically whenever menge/einzelpreis changes
  ...
  const save = async (): Promise<{ id: string; rechnungNr: string; qrReferenz: string | null } | null> => {
    ...
    try {
      if (rechnungId) {
        ...
        // Loaded record: rechnung_nr/qr_referenz state is already current (loadRechnung)
      ...
      // Generate the QR reference from the real rechnung_nr (QR-IBAN → QRR)
      ...
      // setState is async → buildPdfData reads the stale value from state; pass the fresh number via the return value
  ...
  const handleDownload = async () => {
    // save-first: rechnung_nr is generated by the DB trigger on INSERT; take the number from the return value
    const saved = await save();
    if (!saved) return;
