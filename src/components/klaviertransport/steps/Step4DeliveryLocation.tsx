...
export function Step4DeliveryLocation({ data, updateLocation, errors }: Step4Props) {
  const location = data.lieferort || createEmptyLocationDetails();
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-1">📍 Lieferadresse</h3>
        <p className="text-sm text-gray-500">Wohin soll das Instrument geliefert werden?</p>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="col-span-2 space-y-2">
          ...
          <Input
            ...
            className={errors['lieferort.strasse'] ? 'border-red-500' : ''}
          />
          {errors['lieferort.strasse'] && (
            <p className="text-sm text-red-500">{errors['lieferort.strasse']}</p>
          )}
        </div>
        <div className="space-y-2">
          <Label>Nr. *</Label>
          <Input
            ...
            className={errors['lieferort.hausnummer'] ? 'border-red-500' : ''}
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="space-y-2">
          ...
          <Input
            ...
            className={errors['lieferort.plz'] ? 'border-red-500' : ''}
          />
          {errors['lieferort.plz'] && (
            <p className="text-sm text-red-500">{errors['lieferort.plz']}</p>
          )}
        </div>
        <div className="col-span-2 space-y-2">
          <Label>Stadt *</Label>
          <Input
            ...
            className={errors['lieferort.ort'] ? 'border-red-500' : ''}
          />
          {errors['lieferort.ort'] && (
            <p className="text-sm text-red-500">{errors['lieferort.ort']}</p>
          )}
        </div>
      </div>
          ...
          <div className="pl-4 border-l-2 border-blue-200 space-y-4">
            ...
            {(location.lift_passt === 'fits_tight' || location.lift_passt === 'unsure') && (
              <div className="space-y-3">
                <Label className="text-sm text-gray-600">Liftmasse (optional)</Label>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="flex items-center space-x-2">
            <Checkbox
              id="lieferort_enge_tueren"
              checked={location.hindernisse.enge_tueren}
              onCheckedChange={(checked) => updateHindernis('enge_tueren', !!checked)}
            />
            <label htmlFor="lieferort_enge_tueren" className="text-sm">Enge Türen</label>
          </div>
          <div className="flex items-center space-x-2">
            <Checkbox
              id="lieferort_stufen_vor_gebaeude"
              checked={location.hindernisse.stufen_vor_gebaeude}
              onCheckedChange={(checked) => updateHindernis('stufen_vor_gebaeude', !!checked)}
            />
            <label htmlFor="lieferort_stufen_vor_gebaeude" className="text-sm">Stufen vor dem Gebäude</label>
          </div>
          <div className="flex items-center space-x-2">
            <Checkbox
              id="lieferort_schwierige_parkplatz"
              checked={location.hindernisse.schwierige_parkplatz}
              onCheckedChange={(checked) => updateHindernis('schwierige_parkplatz', !!checked)}
            />
            <label htmlFor="lieferort_schwierige_parkplatz" className="text-sm">Schwierige Parkplatzsituation</label>
          </div>
          <div className="flex items-center space-x-2">
            <Checkbox
              id="lieferort_denkmalgeschuetzt"
              checked={location.hindernisse.denkmalgeschuetzt}
              onCheckedChange={(checked) => updateHindernis('denkmalgeschuetzt', !!checked)}
            />
            <label htmlFor="lieferort_denkmalgeschuetzt" className="text-sm">Denkmalgeschütztes Gebäude</label>
          </div>
        </div>
