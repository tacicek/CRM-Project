...
/** Was die Abfrage zurückgibt — dasselbe wie oben, ohne Zustandsfelder. */
type UebersichtErgebnis = Pick<UebersichtData, "workItems" | "activity" | "kpis" | "revenueWeeks">;

/** Feste leere Werte statt `[]` im Render: ein neues Array bei jedem Durchlauf
 *  wäre eine neue Identität und nähme jeder späteren Memoisierung die Grundlage. */
const KEINE_VORGAENGE: WorkItem[] = [];
const KEIN_VERLAUF: ActivityEvent[] = [];
const KEINE_KENNZAHLEN: Kpi[] = [];
const KEINE_WOCHEN: RevenueWeek[] = [];

const isoDay = (date: Date) => format(date, "yyyy-MM-dd");

/**
 * Schlüssel dieser Ansicht.
 *
 * Die Firma gehört in den Schlüssel, nicht nur in die Abfrage: sonst zeigte ein
 * Wechsel der Firma die zwischengespeicherten Zahlen der vorigen.
 */
export const uebersichtQueryKey = (companyId: string | null) =>
  ["uebersicht", companyId] as const;
 ...
 *
 * Über React Query, nicht über `useState` + `useEffect`: sechs Abfragen bei
 * jedem Betreten der Seite neu zu schicken war der eigentliche Grund, warum
 * das Zurückspringen auf die Übersicht sich zäh anfühlte. Der Zwischenspeicher
 * beantwortet den zweiten Besuch sofort und holt im Hintergrund nach.
 */
export const useUebersichtData = (): UebersichtData => {
  const { companyId } = useCompanyContext();

  const abfrage = useQuery({
    queryKey: uebersichtQueryKey(companyId),
    // Ohne Firma gibt es nichts zu holen. React Query wartet dann, statt eine
    // Abfrage mit `null` loszuschicken — und `isPending` bleibt wahr, was dem
    // bisherigen Verhalten entspricht: bis die Firma da ist, stehen Skelette.
    enabled: companyId !== null,
    queryFn: async (): Promise<UebersichtErgebnis> => {
      // `enabled` schliesst den Fall aus; der Typ weiss davon nichts.
      if (companyId === null) throw new Error("Übersicht ohne Firma abgefragt");

      // Die Zeit des Abrufs entsteht hier, nicht draussen: ein von aussen
      // gereichtes Date-Objekt wäre bei jedem Render neu. Die reinen
      // Funktionen nehmen `now` weiterhin als Argument — dort ist es für die
      // Tests nötig.
      const now = new Date();
      ...
      // Vorher verschwand ein Fehler hinter `?? []` — die Seite zeigte dann
      // eine leere Übersicht, als wäre nichts da. Weitergeworfen wird daraus
      // ein Fehlschlag, den React Query einmal wiederholt.
      const fehler =
        leadsResult.error ??
        offersResult.error ??
        auftraegeResult.error ??
      ...
      const activity: ActivityEvent[] = ((verlaufResult.data ?? []) as VerlaufRow[]).map((row) => ({
        id: row.id,
        leadId: row.lead_id,
      ...
      const revenueWeeks = groupPaymentsByWeek(payments, REVENUE_WEEKS, now);
      ...
      return { workItems: items, activity, kpis, revenueWeeks };
    },
  });
  ...
  return {
    workItems: abfrage.data?.workItems ?? KEINE_VORGAENGE,
    activity: abfrage.data?.activity ?? KEIN_VERLAUF,
    kpis: abfrage.data?.kpis ?? KEINE_KENNZAHLEN,
    revenueWeeks: abfrage.data?.revenueWeeks ?? KEINE_WOCHEN,
    // `isPending` und nicht `isFetching`: ein Nachladen im Hintergrund darf die
    // bereits sichtbaren Zahlen nicht durch Skelette ersetzen.
    isLoading: abfrage.isPending,
    reload,
  };
};
